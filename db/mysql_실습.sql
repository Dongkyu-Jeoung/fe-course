...
/**********************************************************
		제약사항(CONSTRAINT) : 데이터 무결성 원칙을 적용하기 위한 규칙
        - UNIQUE 제약사항 : 중복을 방지하는 제약
        - NOT NULL : NULL 값을 허용하지 않는 제약
        - PRIMARY : UNIQUE + NOT NULL
        - FOREIGN : 타 테이블의 기본키를 참조
        - DEFAULT : 데이터 입력 시 기본으로 저장되는 데이터 설정
***********************************************************/
...
-- EMP_CONST 테이블 생성, 기본키(PRIMARY KEY) 제약, 참조키(FOREIGN), NOT NULL
...
SHOW TABLES;
SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'EMP_CONST2';
...
-- PHONE 컬럼의 NULL값을 '010-1111-1234' 으로 변경
UPDATE EMP_CONST
	SET PHONE = '010-1111-1234';
SELECT * FROM EMP_CONST;

-- PHONE 컬럼에 DEFAULT 값 '010-1111-1234' 지정
ALTER TABLE EMP_CONST
	ALTER COLUMN PHONE SET DEFAULT '010-1111-1234';
DESC EMP_CONST;

INSERT INTO EMP_CONST(EMP_ID, EMP_NAME, HIRE_DATE)
	VALUES('S003', '홍길동', NOW());
    
-- SALARY 컬럼에 DEFAULT 제약 추가 (1000)
ALTER TABLE EMP_CONST
	ALTER COLUMN SALARY SET DEFAULT 1000;


ALTER TABLE EMP_CONST
	ALTER COLUMN HIRE_DATE SET DEFAULT '2026-01-01';

INSERT INTO EMP_CONST(EMP_ID, EMP_NAME)
	VALUES('S004', '김유신');
    
-- CHECK 제약 : MYSQL 8.0 이상부터
-- EMP_CONST3 테이블의 HIRE_DATE 컬럼은 DEFAULT 제약 추가, '2026-04-01'
-- SALARY 컬럼은 NOT NULL 제약
...
-- SALARY 값을 입력시 3000 이상인 값만 저장되도록 CHECK 제약사항 추가
ALTER TABLE EMP_CONST3
...
INSERT INTO EMP_CONST3 (EMP_ID, EMP_NAME, SALARY)
	VALUES('S001', '홍길동', 3000);

-- EMP_CONST3 테이블에 EMP_NAME 컬럼에 UNIQUE 제약 추가
ALTER TABLE EMP_CONST3
	MODIFY EMP_NAME VARCHAR(5) UNIQUE;
...
-- EMP_CONST2 테이블의 DEPT_ID 컬럼이 DEPARTMENT2의 DEPT_ID를 참조하도록
ALTER TABLE EMP_CONST2
	ADD CONSTRAINT FK_EMP_CONST2_DEPT_ID FOREIGN KEY(DEPT_ID) REFERENCES DEPARTMENT2(DEPT_ID);
    
SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE TABLE_NAME = 'EMP_CONST2';

INSERT INTO EMP_CONST2(EMP_ID, EMP_NAME, DEPT_ID)
	VALUES('S001', '홍길동', 'SYS');
INSERT INTO EMP_CONST2(EMP_ID, EMP_NAME, DEPT_ID)
	VALUES('S002', '이순신', 'ACC');
...
-- DEPARTMENT2 테이블의 SYS 부서 삭제? -> EMP_CONST2에서 참조하고있으니까 삭제 안됨
-- (1) EMP_CONST2 테이블의 SYS 참조 행을 삭제
-- (2) DEPARTMENT2 테이블의 SYS 데이터 삭제 가능

-- (1)
DELETE FROM EMP_CONST2 WHERE DEPT_ID = 'SYS';
-- (2)
DELETE FROM DEPARTMENT2 WHERE DEPT_ID = 'SYS';
SELECT * FROM DEPARTMENT2;

-- 참조하는 부모테이블의 컬럼이 변화함에 따라 자식도 함께 자식도 적용받도록 옵션 정의
-- ON [DELETE/UPDATE] CASCADE; 		참조키 제약 정의시 마지막에 추가

ALTER TABLE EMP_CONST2
	DROP CONSTRAINT FK_EMP_CONST2_DEPT_ID;
    
SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS WHERE TABLE_NAME = 'EMP_CONST2';

ALTER TABLE EMP_CONST2
	ADD CONSTRAINT FK_EMP_CONST2_DEPT_ID FOREIGN KEY(DEPT_ID) REFERENCES DEPARTMENT2(DEPT_ID) ON DELETE CASCADE ON UPDATE CASCADE;
    
SELECT * FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE WHERE TABLE_NAME = 'EMP_CONST2';
    
-- DEPARTMENT2의 'ACC'부서를 'ABC' 부서로 수정
UPDATE DEPARTMENT2 SET DEPT_ID = 'ABC'
WHERE DEPT_ID = 'ACC';
